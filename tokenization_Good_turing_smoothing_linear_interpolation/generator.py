...
def predict_next_words(next_predict_sentence, ngramToProbs, nMinusTwoGramToCount, top_n=5):

    tokenizer = Tokenizer()
    user_input = next_predict_sentence
    if user_input[-1] not in ".!?;":
        user_input += "."  
    user_input = tokenizer.preprocess(user_input)
    processed_user_input_sentences = tokenizer.sentencize( user_input)
    ...
    words = tokenized_user_input_sentences[0]
    second_last_word = words[-1]
    third_last_word = words[-2]
    last_two_words = (third_last_word, second_last_word)

    # print("last 2 words:" , last_two_words)

    matching_ngrams = {k: v for k, v in ngramToProbs.items() if k[0] == last_two_words}

    sorted_ngrams = sorted(matching_ngrams.items(), key=lambda x: x[1], reverse=True)

    top_predictions = sorted_ngrams[:top_n]

    total_weight = sum(item[1] for item in top_predictions)
    weighted_average = [item[1]  / total_weight for item in top_predictions]

    for i in range(len(top_predictions)):
        top_predictions[i] = (top_predictions[i][0], weighted_average[i])

    if len(top_predictions) < top_n:
        top_unigrams = [(unigram, nMinusTwoGramToCount[unigram]) for unigram in nMinusTwoGramToCount]
        top_unigrams = sorted(top_unigrams, key=lambda x: x[1], reverse=True)
        remaining_vals = top_unigrams[:top_n - len(top_predictions)]

        total_weight = sum(item[1] for item in remaining_vals)
        weighted_average = [item[1]  / total_weight for item in remaining_vals]

        for i in range(len(remaining_vals)):
            remaining_vals[i] = (remaining_vals[i][0], weighted_average[i])

        top_predictions += remaining_vals
...
# selected_sentences = random.sample(processed_user_input_sentences, min(len(processed_user_input_sentences), 1000))


# remaining_sentences = [sentence for sentence in processed_user_input_sentences if sentence not in selected_sentences]
...
next_predict_sentence = input("input sentence: ")
top_predictions = predict_next_words(next_predict_sentence, ngramToProbs,nMinusTwoGramToCount,top_n)
for i in range(0, len(top_predictions)):
    print(top_predictions[i][0][1]," ",top_predictions[i][1] )




# def test_model_ood( ood_text):
    
    
#     perplexity = getPerplexity(ood_text, n, ngramToCount, ngramToProbs, unseenProb, showlog=False)
#     top_predictions = predict_next_words(ood_text, ngramToProbs,nMinusTwoGramToCount,top_n)
#     for i in range(0, len(top_predictions)):
#         print(top_predictions[i][0][1]," ",top_predictions[i][1] )
    
#     print(f"Perplexity of the OOD text: {perplexity}")
...
# test_model_ood("mhgnfddgxfbf gdf fdxgf mngfd mghnfg fngbfv")

# test_model_ood("afsdg fsdgf fdg fesd gsd esgd")

# test_model_ood("trhdb fesggssdv asfd mngb ed gfvxc")

# test_model_ood("wefsd fdgb asdg bnvbc bxcn oiuyt")
    


# ngramToProbs = {}
# numberOfTokens = sum(nMinusTwoGramToCount.values())
# # Calculate probabilities for each n-gram
# for ngram, count in ngramToCount.items():
#     ngramToProbs[ngram] = count / numberOfTokens
...
# def predict_next_word(next_predict_sentence, bigramToProbs, unigramToCount, top_n=5):
#     tokenizer = Tokenizer()
#     user_input = next_predict_sentence
#     if user_input[-1] not in ".!?;":
#         user_input += "."
...
#     matching_bigrams = {k: v for k, v in bigramToProbs.items() if k[0] == last_word}

#     sorted_bigrams = sorted(matching_bigrams.items(), key=lambda x: x[1], reverse=True)

#     top_predictions = sorted_bigrams[:top_n]
...
#     if len(top_predictions) < top_n:
#         top_unigrams = [(unigram, unigramToCount[unigram]) for unigram in unigramToCount]
#         top_unigrams = sorted(top_unigrams, key=lambda x: x[1], reverse=True)
#         remaining_vals = top_unigrams[:top_n - len(top_predictions)]
...
# numberOfTokens = sum(nMinusTwoGramToCount.values())
# # Calculate probabilities for each n-gram
# for ngram, count in nMinusOneGramToCount.items():
#     biGramToProbs[ngram] = count / numberOfTokens


# print()
# next_predict_sentence = input("input sentence: ")
# top_predictions = predict_next_word(next_predict_sentence, biGramToProbs,nMinusTwoGramToCount,top_n)
# for i in range(0, len(top_predictions)):
#     print(top_predictions[i][0][1]," ",top_predictions[i][1] )